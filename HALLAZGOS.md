...
## Nota metodológica: corrección de un sesgo de muestra y replicación

Una revisión posterior detectó que la muestra original de 24 agentes (usada en todos
los experimentos anteriores) tenía una distribución de voto_2023 más desbalanceada de
lo previsto por los pesos programados: 11 UxP contra solo 6 LLA, una brecha de 19
puntos porcentuales muy superior a la diferencia real entre Massa (36,6%) y Milei
(29,9%) en las elecciones generales de octubre de 2023 (~7 puntos). Esto importa
porque uno de los hallazgos del Experimento 2 — que la periferia que se mueve es
mayormente ex-votantes de UxP — podía ser, al menos en parte, un artefacto de que
había casi el doble de agentes UxP que LLA en la muestra, no necesariamente una
propiedad real de cómo razona cada bloque.

Se corrigieron los pesos de generación (ahora reflejando ~33/29/22% para UxP/LLA/JxC,
en línea con el resultado real de octubre 2023) y se regeneró la muestra completa con
una semilla distinta, obteniendo una distribución balanceada: 7 LLA / 7 UxP / 6 JxC /
3 blanco / 1 otro.

### Resultado de la replicación del Experimento 2 con la muestra corregida

El patrón estructural se replicó con fuerza: **12 de 24 agentes cambiaron en la Ronda
1 ($LIBRA), seguido de 0 cambios en cada una de las tres rondas siguientes** (ANDIS,
derrota PBA, síntesis Financial Times) — exactamente la misma forma que en la muestra
original (que había dado 6/24, después 0/0/0), pero con el doble de magnitud de
movimiento inicial (50% en vez de 25%).

**Esto confirma que el patrón central — todo el movimiento ocurre en el primer
evento, el resto del desgaste no perfora nada adicional — no depende del desbalance
de partidos que se corrigió.** Se sostiene con una composición de muestra distinta,
otra semilla aleatoria, y otro reparto de partidos.

**Lo que no puede afirmarse con la misma confianza** es la causa del salto en
magnitud (de 6 a 12 agentes). La nueva tirada aleatoria no solo balanceó los
partidos: también salió con una composición ocupacional más vulnerable (más
desempleados, más comercio informal) que la muestra original. Como ambas variables
cambiaron juntas en la misma regeneración, no es posible aislar con este diseño si el
salto de intensidad se debe al rebalanceo partidario, a la composición
socioeconómica, o a ambas cosas combinadas. Aislar esa causa específica requeriría
una tercera muestra que mantenga el balance de partidos pero replique la composición
socioeconómica original — un experimento adicional no realizado en esta fase.
...
## Experimento 4: contrafactual — ¿importa CUÁL fue el primer escándalo?

Pregunta motivadora: Milei construyó buena parte de su capital político sobre una
promesa específica (anti-casta, anti-corrupción). $LIBRA fue el primer evento que
rompió esa promesa, con el agravante de tener al propio presidente como protagonista
directo (no un funcionario de segunda línea). ¿Es ese protagonismo y esa ruptura
fundacional lo que explica que Libra fuera el único evento con movimiento real en el
Experimento 2 — o hubiera bastado cualquier primer escándalo de corrupción, sea cual
sea su contenido específico, para producir el mismo efecto?

### Diseño

Se creó una rama paralela de los mismos 24 agentes, con historial limpio, y se la
expuso a la secuencia real **sin $LIBRA**: el primer evento que estos agentes
conocen es directamente el escándalo ANDIS (agosto-2025), presentado como si fuera
la primera vez que el agente se entera de un caso de corrupción del mandato.

### Resultado: parcialmente Hipótesis B, con un matiz que la hace más interesante

La rama contrafactual (ANDIS primero) produjo **4 cambios sobre 24**, muy similar en
magnitud a los 6 cambios que produjo Libra en la rama original, y el equilibrio
final por partido (6 UxP / 6 LLA / 2 JxC) fue **idéntico** entre ambas ramas — solo
varió levemente el reparto entre voto en blanco y "otro/provincial".

Comparando agente por agente quién cambió en cada rama:
- **Rama original (Libra):** agente_02, 08, 10, 19, 21, 24.
- **Rama contrafactual (ANDIS):** agente_15, 19, 21, 24.
- **Coincidencia exacta: agente_19, 21 y 24** cruzaron en ambas ramas,
  independientemente de cuál fue el primer escándalo.

Esto sugiere una estructura de dos capas, más matizada que una respuesta binaria:

1. **Un "núcleo de periferia" estructuralmente sensible al primer quiebre de la
   promesa anti-corrupción, sea cual sea su contenido específico** (agentes 19, 21,
   24) — lo que importa para estos tres no es el evento en sí, sino la función que
   cumple (primera ruptura de una promesa fundacional), consistente con la
   Hipótesis B.
2. **Un margen adicional cuya reacción sí depende del contenido específico del
   evento** (agente_02, 08, 10 reaccionaron a Libra pero no a ANDIS; agente_15
   reaccionó a ANDIS pero no a Libra) — sugiriendo que ciertos eventos resuenan más
   con ciertos perfiles según cómo se relacionan con su situación particular (por
   ejemplo, agente_15, un cuentapropista preocupado por costos e impuestos, podría
   conectar más con el desvío de fondos hacia discapacidad que con un escándalo
   cripto más fácil de encasillar como "cosa de especuladores").

### Decisión de cierre

No se completaron las dos rondas restantes de la rama contrafactual (PBA, síntesis
FT) porque, dado que esos mismos eventos produjeron cero cambios en la rama original,
no había razón para esperar un resultado distinto en la rama alternativa - agregar
esas corridas no iba a aportar información nueva proporcional al costo. Se prioriza
el criterio de parar cuando una corrida adicional deja de ser informativa, en vez de
completar el diseño por completitud formal.

### Conclusión de este experimento, sin sobre-interpretar

Con un solo punto de comparación (un evento sustituido, no una serie completa), esto
es sugerente pero no se puede generalizar como ley. Lo que sí puede afirmarse: el
"quién" protagoniza el escándalo (Milei vs. un funcionario) parece pesar menos de lo
esperado intuitivamente — lo que más explica el movimiento observado es la posición
estructural del evento en la secuencia (ser el primer quiebre) combinada con una
resonancia específica entre el contenido del evento y la situación particular de
cada agente, no una sola de esas dos cosas por separado.
